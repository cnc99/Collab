...
#Variation of rippe_nonparallel without the rolling friction
...
#bball is the one i want to use
"""
...
def load_experiment(fname="effData.txt", get_eff_data=False):
    ...
    # measures how much the object moved in the x and y directions
    diffx = (df['finalObjectPos[0]'] - df['initialObjPos[0]'])
    diffy = (df['finalObjectPos[1]'] - df['initialObjPos[1]'])

    # matrix with the mean 'movements' in the x and y direction
    mvec = np.array([diffx.mean(), diffy.mean()])
    # mean 'movements' in both directions
...
# connects to simulation, sets gravity force for all objects
# and loads physics model
def call_simulator():
    # print("start sim")
...
def gen_run_experiment(pbar, param_names, object_name="ylego", tools=("rake", "stick"), #, "hook"),
    ...
    #objID = p.createCollisionShape(p.GEOM_SPHERE, radius=0.057) # primitive shape
    objID = load_object()
        ...
        for tool_name in tools:
            for action_name in actions:
                target_pos, target_var, gnd_weight, mdist, real_eff_history = load_experiment(
                    "affordance-datasets/visual-affordances-of-objects-and-tools/{}/{}/{}/effData.txt".format(tool_name,
                                                                                                              object_name,
                                                                                                              action_name),
                    get_eff_data=True)
                ...
                for iter in range(N_EXPERIMENTS):
                    ...
                    while not success:
                        ...
                        try:
                            ...
                            yaw = 0 #orientations_list[iter][0]
                            pitch = 0 #orientations_list[iter][1]
                            roll = orientations_list[iter] #orientations_list[iter][2]
                            ...
                            #dic_params['lateralFriction']=3.7702
                            # dic_params['spinningFriction']=0.0001
                            #dic_params['rollingFriction']=0.000606869
                            ...
                            p.resetBasePositionAndOrientation(objID, posObj=[initial_xy[0], initial_xy[1],
                            ...
                            file = open("simulated-dataset/{}/{}/{}/effData_OR_v2.txt".format(tool_name, object_name,action_name), "a+")
                            file.write(str(tool_name) + " " +str(object_name) + " " + str(actionId) + " " + str(initial_xy[0]) + " " + str(initial_xy[1]) +
                                       " " + str(-1) + " " + str(-1) + " " + str(pos[0]) + " " + str(pos[1]) + " " + str(-1) + " " + str(-1) + " " + str(0) + " " + str(0) + " " + str(orientations_list[iter]) + " " + "\n")
                            file.close()
                ...
                if PLOTTING:
                    ...
                    plt.savefig("plots/{}.pdf".format(kld))
                    plt.show()
...
def optimize(param_names, fname, object_name="bball", opt_f=gp_minimize):
...
if __name__ == "__main__":
    ...
    call_simulator()
    param_names = ['lateralFriction', 'mass']  # , 'xnoise', 'ynoise']
