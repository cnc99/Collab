...
N_EXPERIMENTS = 2  # running experiment per object per tool
N_TRIALS = 10  # optimization steps
...
def gen_run_experiment(pbar, param_names, object_name="bball", tools=("rake", "stick", "hook"),
    ...
    objID = p.createCollisionShape(p.GEOM_SPHERE, radius=0.057)

    def f(params):
        dic_params = {pname: param for pname, param in zip(param_names, params)}
        offset = 0.01 if object_name == "bball" else 0.0
